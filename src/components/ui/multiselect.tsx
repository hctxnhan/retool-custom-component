...
interface MultipleSelectorProps {
  ...
  creatable?: boolean
  /** Allow user to create custom values from input */
  allowCustom?: boolean
  /** Callback when a custom option is created */
  onCreateOption?: (inputValue: string) => Option | null
  /** Props of `Command` */
...
>(
  (
    ...
    const CreatableItem = () => {
      if (!creatable && !allowCustom) return undefined
      if (
        ...
        <CommandItem
          ...
          onSelect={(value: string) => {
            ...
            if (allowCustom && onCreateOption) {
              // Use the custom create option function
              const customOption = onCreateOption(value)
              if (customOption) {
                const newOptions = [...selected, customOption]
                setSelected(newOptions)
                onChange?.(newOptions)
                return
              }
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    option.isCustom && 'border-primary/30',
                    badgeClassName
                  )}
